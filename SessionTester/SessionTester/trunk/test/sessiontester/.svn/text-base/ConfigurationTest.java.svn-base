...
public class ConfigurationTest extends TestCase {
    ...
    public void testPrimingList() throws IOException {
        Configuration configuration = Configuration.BuildConfiguation();
        assertEquals(93, configuration.getPrimingList().size());
        
        for (String idea : configuration.getPrimingList()) {
            assertTrue(null != idea && !idea.isEmpty());
        }
    }
